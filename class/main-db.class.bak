...
*/
        ...
        $mysqlConn = new mysqlConn("localhost", "adbdba", "iv3gqfsxt3n9", "adbdba_anniedebrowsa"); 
 	...
 	*/
 	die("Yo, mysqli_error(".mysqli_error($mysqlConn->dblinkObj).") in ".$_SERVER['PHP_SELF']." at line ". __LINE__ .".");
        ...
        if (!$result) {
            ...
            $htmlShow .= 'MySQL Error: ' . mysqli_error();
            exit;
        }
...
// END domainTables->showTables
    ...
    function dropSelect($table, $field, $other) { 
/** @method dropSelect
...
*/
        ...
        $htmlShow['html']= array('html' => "", 'newurl' =>"", 'comm' => "");

        if(!$result){
            $htmlShow['html'] .= "DB Error, could not list tables\n";
            $htmlShow['html'] .= 'MySQL Error: ' . mysqli_error();
            exit;
        }
        else {
...
// END domainTables->dropSelect
    ...
    function urlNames($table, $field, $other) {
        require_once "d-blink-inc.php";
        $mysqlConn = new mysqlConn("localhost", "adbdba", "iv3gqfsxt3n9", "adbdba_anniedebrowsa");

        $sql = "SELECT ".$field." FROM `".$table."` ".
            "ORDER BY `url_name` ASC ;";
        $result = mysqli_query($sql) or die("Yo, I got stuck on this:".mysqli_error()." in ".$_SERVER['PHP_SELF']." at line ". __LINE__ .".");
        $numRows = mysqli_num_rows($result);

        $htmlShow['html']= array('html' => "", 'newurl' =>"");
...
// END domainTables->urlNames
    ...
    function getUrlDetails($dbName, $table, $urlid) {
        require_once "d-blink-inc.php";
        $mysqlConn = new mysqlConn("localhost", "adbdba", "iv3gqfsxt3n9", "adbdba_anniedebrowsa");
...
// END domainTables->getUrlDetails
    ...
    function updateUrl($table, $urlid, $addr, $auth, $comm, $name, $tags) {
        ...
        $insertResult = mysqli_query($insertQue) or die("Yo, I got stuck on this:".mysqli_error()." in ".$_SERVER['PHP_SELF']." at line ". __LINE__ .".");
...
// END domainTables->updateUrl
    ...
    function insertNewurl($table, $addr, $auth, $comm, $name, $tags) {
        require_once "d-blink-inc.php";
        $mysqlConn = new mysqlConn("localhost", "adbdba", "iv3gqfsxt3n9", "adbdba_anniedebrowsa");
        $insertQue = "INSERT INTO `".$table."`".
            " (`url_addr`, `url_auth`, `url_comm`, `url_name`, `url_tags`) VALUES ".
            " ('".$addr."', '".$auth."', '".$comm."', '".$name."', '".$tags."');";
        $insertResult = mysqli_query($mysqlConn->dblinkObj,$insertQue) or die("Yo, I got stuck on this:<br />".mysqli_error()." in ".$_SERVER['PHP_SELF']." at line ". __LINE__ .".");
...
// END domainTables->deleteRow
    ...
    function reviewNames($table, $field, $other) {
        require_once "d-blink-inc.php";
        $mysqlConn = new mysqlConn("localhost", "adbdba", "iv3gqfsxt3n9", "adbdba_anniedebrowsa");
        $sql = "SELECT ".$field." FROM `".$table."` ".
            "ORDER BY $other ASC ;";
