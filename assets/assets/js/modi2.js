...
function so_init() {
	...
	outlineObj = d.getElementsByTagName("body")[0].appendChild(d.createElement("div"));
	outlineObj.id = "so_outlineObj";
	outlineObj.style.borderColor = pref_outlineColor;
	outlineObj.style.borderWidth = pref_outlineWidth;
...
function so_buildDataDisplay(obj) {
	...
	// computed styles
	h3 = sObj.appendChild(d.createElement("h3"));
	h3.appendChild(d.createTextNode("computed styles"));
	
	ul = sObj.appendChild(d.createElement("ul"));
	ul.setAttribute("id","styleData");
	ul.style.maxHeight="100px";
	ul.style.overflow="scroll";
	
	so_getObjStyles(obj,ul);
	...
	fr = sObj.appendChild(d.createElement("iframe"));
...
function so_getObjStyles(obj,pObj) {

	function reformatString(str) {
		tmp="";
		for(z=0;z<str.length;z++) {
  			if(str.charCodeAt(z)>=65 && str.charCodeAt(z)<=90) {
   				tmp+="-"+str.charAt(z).toLowerCase();
  			} else {
   				tmp+=str.charAt(z);
  			}
 		}
	...
	if(!d.all || window.opera){
		x = document.defaultView.getComputedStyle(obj,null);
		m="";
		for(prop in x) {
			if(x.getPropertyValue(reformatString(prop))) {
				//m+=reformatString(prop) + ":" + x.getPropertyValue(reformatString(prop))+";<br />";
				li = pObj.appendChild(d.createElement("li"));
				li.appendChild(d.createTextNode(reformatString(prop) + ":" + x.getPropertyValue(reformatString(prop)) + ";"));
			}
		}
	} else {
		x = document.getElementsByTagName("input")[0];
		m="";
		for(prop in obj.currentStyle) {
			if(obj.currentStyle[prop]) {
				//m+= prop + ":" +obj.currentStyle[prop] +";<br />";
				li = pObj.appendChild(d.createElement("li"));
				li.appendChild(d.createTextNode(prop + ":" + currentStyle[prop] + ";"));	
			}
		}
	}
