...
class UrlChopper {
	...
	function __construct($passpath){
		...
		*/
		...
		$this->whychop = $passpath ? $passpath : ADBLOCTN;
	...
	function chopUrl(){
		...
		*/
		...
		$this->chop_url		= str_ireplace($result,$replace,$subject);
		$this->chop_trim = ltrim($result,"/");
		$chop_trim = ltrim($this->chop_url,"/");

		return preg_replace("/.*\//",'',$this->chop_url);
	}
	// END METHOD chopUrl

	function svgTextChopper(){
		/*
		* @workinprogress
