...
if(preg_match('@file@i',$switchVal,$segMatch)){
    ...
    $path2url .= '/'.$switchVal;;
    $htmlConcat .= '<br /><img src="'.$xro.'assets/css/warning_transp.png" alt="Attention!" /> key['.$i.']: <code>preg_match(&#x27;/'.$segMatch[0].'/i&#x27;, &#x27;<strong class="red">'.$switchVal.'</strong>&#x27;)</code>'."\n";
...
elseif(($i < 1) && preg_match('@[\w_\-]+@i',$switchVal,$segMatch)) {
    ...
    $test[$i] = __LINE__;
    $test_error_line = $test[$i];
    $htmlConcat .= '<br /><br /><img src="'.$xro.'assets/css/stop.gif" alt="ERROR!" /> key['.$i.']:<br />
...
$test[$i] = __LINE__;
$path2url .= '/'.$switchVal;
$htmlConcat .= '<br /><img src="'.$xro.'assets/css/check_green.png" alt="Valid!" /> key['.$i.']: <span class="urlPathNoMatch">'.$switchVal.'</span> was used to build the HTTP URL.';
$htmlConcat .= '<br /><span class="serif">result: <span class="green monospace urlSoFar">'.$path2url.'</span> &#x2026;</span>
        ';
}
    } // end test_segment()
