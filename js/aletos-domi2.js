...
function charm(){
	...
	function createDoc1(){
		win=window.open('','','top='+window.innerHeight*0.05+',height='+
			window.innerHeight*0.9+',width='+windowWidth+', toolbar = no, directories = no, status = no, menubar = no, resizable=yes, scrollbars = yes, location=no');
		win.document.open();
		win.document.write(''+
...
var elDispatch = ele.dispatchEvent(event);
